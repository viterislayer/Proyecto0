// ✅ FIX 405: Apuntar al backend de Azure, no a GitHub Pages
const API_URL = 'https://TU-APP.azurewebsites.net/api'; // 👈 CAMBIA ESTO POR TU URL REAL DE AZURE
...
(function injectMobileStyles() {
    ...
    style.innerHTML = `
        @media (max-width: 768px) {
            .modal-content.glass { padding: 24px 18px !important; width: 96% !important; }
            ...
            .navbar { padding: 14px 18px !important; gap: 12px !important; flex-direction: column !important; align-items: stretch !important; }
            .navbar .logo { font-size: 1.1rem !important; }
            .navbar div[style*="flex-wrap:wrap"] { justify-content: center; width: 100%; gap: 10px; }
            .navbar button { padding: 10px 14px !important; font-size: 0.78rem !important; width: 100% !important; max-width: 260px !important; }
            .avatar { width: 34px !important; height: 34px !important; }
            h1, h2 { font-size: 1.4rem !important; }
            .btn-premium { padding: 12px 16px !important; font-size: 0.88rem !important; }
            .grid { grid-template-columns: 1fr !important; padding: 12px !important; gap: 16px !important; }
            .job-card, .profile-card { min-height: auto !important; }
            .job-card > div, .profile-card > div { padding: 18px 16px !important; }
            .modal { padding: 8px !important; }
            .modal button, .modal .btn-premium, .modal .btn-ghost { width: 100% !important; }
            .cropping-modal-content, .formal-modal-content { max-width: 480px !important; width: 100% !important; padding: 22px !important; }
            .btn-ghost { min-height: 44px !important; }
            #registroForm .glass, #loginForm .glass { padding: 32px 22px !important; }
        }
        input, select, textarea { font-size: 16px !important; } /* Evita zoom automático en iOS */
        .input-group { position: relative; } /* Anclaje para elementos absolutos */
        .password-toggle {
            position: absolute;
            right: 10px;
            top: 50%;
        ...
        .password-toggle:hover { color: #0a84ff; background: rgba(10, 132, 255, 0.1); }
    `;
    document.head.appendChild(style);

    window.togglePass = (name) => {
        const inp = document.querySelector(`input[name="${name}"]`);
        const ico = inp.parentElement.querySelector('.password-toggle i');
...
function openJobDetails(id) {
    ...
    const empresaFoto = (item.empresa_foto && item.empresa_foto.startsWith('http')) ? item.empresa_foto : '/uploads/' + (item.empresa_foto || 'default.jpg');
    ...
    const requisitos = item.requisitos || item.requisitos_clave || 'Experiencia previa, buena comunicación y actitud proactiva.';
    const beneficios = item.beneficios || item.lo_que_ofrecemos || 'Trabajo en equipo, horario flexible, capacitaciones y ambiente agradable.';
...
function openCandidateDetails(id) {
    ...
    const cFoto = (item.foto_perfil && item.foto_perfil.startsWith('http')) ? item.foto_perfil : '/uploads/' + (item.foto_perfil || 'default.jpg');
                            ...
                            <div class="glass job-card profile-card" style="animation-delay:${i * 0.06}s;border:1.5px solid ${isDisponible ? 'rgba(10,132,255,0.25)' : 'rgba(255,107,99,0.45)'};background:${isDisponible ? 'rgba(255,255,255,0.03)' : 'rgba(255,107,99,0.07)'};box-shadow: 0 12px 40px -10px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(10,132,255,0.03); ${isOwner ? 'height:100%;' : 'align-self:start; height:auto; min-height:auto;'}">
                                <div style="padding:${isOwner ? '24px 22px' : '24px 22px 24px'};display:flex;flex-direction:column;gap:0;${isOwner ? 'min-height:100%;height:100%;' : 'height:auto; min-height:auto;'}">
                                    ...
                                    <div style="text-align:center;margin-bottom:6px;">
                                        ...
                                        <div style="font-size:0.8rem;color:rgba(255,255,255,0.5);font-weight:700;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:14px;">${item.ubicacion || 'Oficina'}</div>
                                        ${item.requisitos ? `<div style="font-size:0.72rem;color:rgba(255,255,255,0.55);line-height:1.4;margin-bottom:12px;">Requisitos: <strong style=\"color:#fff;font-weight:700;\">${item.requisitos.length > 60 ? item.requisitos.slice(0,60) + '...' : item.requisitos}</strong></div>` : ''}
                                        ${item.beneficios ? `<div style="font-size:0.72rem;color:rgba(255,255,255,0.55);line-height:1.4;margin-bottom:18px;">Ofrecemos: <strong style=\"color:#fff;font-weight:700;\">${item.beneficios.length > 60 ? item.beneficios.slice(0,60) + '...' : item.beneficios}</strong></div>` : ''}
