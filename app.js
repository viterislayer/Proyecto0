    ...
    // ── DASHBOARD ──
        ...
        app.innerHTML = `
            ...
            <div style="max-width:${IS_MOBILE ? '100%' : '1400px'};margin:auto;padding:${IS_MOBILE ? (currentUser.rol === 'usuario' ? '12px 12px 84px' : '12px') : '32px 24px'};">
                ...
                <div class="${IS_MOBILE ? '' : 'grid'}" style="${IS_MOBILE ? 'display:grid;grid-template-columns:repeat(2,1fr);gap:10px;padding:14px 0;grid-auto-rows:370px;' : ''}">
                    ${feedItems.map((item, i) => {
                        if(item.itemType === 'job') {
                            ...
                            if (IS_MOBILE) return `
                            <div class="glass" data-candidate-id="${item.id}" style="border-radius:20px;padding:14px 12px;display:flex;flex-direction:column;align-items:center;text-align:center;gap:9px;animation-delay:${i*0.05}s;border:1px solid rgba(255,215,0,0.15);position:relative;overflow:hidden;height:100%;justify-content:space-between;">
