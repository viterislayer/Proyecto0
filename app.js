...
function rechazarPostulante(postulacion_id, nombre) {
    if (!confirm(`¿Rechazar la postulación de ${nombre}?`)) return;
    fetch(`${API_URL}/postulaciones/${postulacion_id}/estado`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        body: JSON.stringify({ estado: 'rechazado' })
    }).then(r => r.json()).then(() => {
        notify('Listo', `Postulación de ${nombre} rechazada.`, 'error');
        render('misPostulacionesEmpresa');
    }).catch(() => notify('Error', 'No se pudo actualizar.', 'error'));
}

function aceptarPostulante(postulacion_id, nombre) {
    const overlay = document.createElement('div');
    overlay.id = 'aceptarModal';
    Object.assign(overlay.style, { position: 'fixed', inset: '0', background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: '20000', padding: '20px' });
    overlay.innerHTML = `
        <div class="glass" style="max-width:440px;width:100%;padding:36px;border-radius:24px;border:1px solid rgba(48,209,88,0.25);box-shadow:0 30px 80px rgba(0,0,0,0.7);">
            <div style="text-align:center;margin-bottom:24px;">
                <div style="width:60px;height:60px;background:rgba(48,209,88,0.12);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;border:1.5px solid rgba(48,209,88,0.3);">
                    <i class="fas fa-check-circle" style="color:#30d158;font-size:1.7rem;"></i>
                </div>
                <h3 style="font-size:1.3rem;font-weight:900;color:#fff;margin-bottom:6px;">Aceptar a ${nombre}</h3>
                <p style="color:rgba(255,255,255,0.5);font-size:0.88rem;">Ingresa un medio de contacto para que el candidato pueda comunicarse contigo.</p>
            </div>
            <div style="margin-bottom:20px;">
                <label style="display:block;font-size:0.72rem;color:#30d158;font-weight:700;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:8px;margin-left:14px;">Teléfono o correo de contacto</label>
                <div class="input-group" style="margin:0;">
                    <i class="fas fa-address-card" style="color:#30d158;"></i>
                    <input id="contactoInput" placeholder="Ej: 50212345678 o empresa@correo.com" style="border-color:rgba(48,209,88,0.2);">
                </div>
            </div>
            <div style="display:flex;gap:10px;">
                <button id="cancelarAceptarBtn" class="btn-premium btn-ghost" style="flex:1;padding:14px;justify-content:center;">Cancelar</button>
                <button id="confirmarAceptarBtn" class="btn-premium" style="flex:2;padding:14px;justify-content:center;background:linear-gradient(135deg,#30d158,#25a244);box-shadow:0 8px 22px rgba(48,209,88,0.3);">
    ...
    document.getElementById('cancelarAceptarBtn').onclick = () => overlay.remove();
    document.getElementById('confirmarAceptarBtn').onclick = () => {
        const contacto = document.getElementById('contactoInput').value;
        confirmarAceptar(postulacion_id, nombre, contacto);
    };
}

async function confirmarAceptar(postulacion_id, nombre, contacto) {
    if (!contacto.trim()) { notify('Aviso', 'Ingresa un teléfono o correo de contacto.', 'warning'); return; }
    try {
        const r = await fetch(`${API_URL}/postulaciones/${postulacion_id}/estado`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
            body: JSON.stringify({ estado: 'aceptado', mensaje_contacto: contacto.trim() })
        });
        const data = await r.json().catch(() => ({}));
        if (!r.ok) {
            console.error('Error al aceptar:', r.status, data);
            notify('Error', data.error || `Error ${r.status}`, 'error');
            return;
        }
        document.getElementById('aceptarModal')?.remove();
        notify('¡Aceptado!', `${nombre} ha sido aceptado y notificado.`);
        render('misPostulacionesEmpresa');
    } catch(e) {
...
function mobileBottomNav(active) {
    ...
    const items = [
        ...
        isEmpresa
            ...
            : { id: 'postulaciones', icon: 'fa-file-alt', label: 'Mis Solicitudes', action: "render('misPostulaciones')" },
        { id: 'perfil', icon: 'fa-user', label: 'Perfil', action: "render('perfil')" },
        { id: 'soporte', icon: 'fa-headset', label: 'Ayuda', action: "render('soporte')" }
    ];
    return `
    <nav style="position:fixed;bottom:0;left:0;right:0;background:rgba(7,7,15,0.95);backdrop-filter:blur(30px);border-top:1px solid rgba(255,255,255,0.08);display:flex;justify-content:space-around;align-items:center;padding:10px 0 calc(10px + env(safe-area-inset-bottom));z-index:200;">
        ${items.map(it => `
        <button onclick="${it.action}" style="background:none;border:none;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:4px;padding:6px 14px;border-radius:14px;transition:0.2s;position:relative;${active === it.id ? 'color:#0a84ff;' : 'color:rgba(255,255,255,0.45);'}">
            <i class="fas ${it.icon}" style="font-size:1.3rem;"></i>
            ${it.id === 'postulaciones' && !isEmpresa ? `<span class="notif-badge-postulaciones" style="position:absolute;top:0;right:8px;background:#ff3b30;color:#fff;border-radius:50%;min-width:16px;height:16px;font-size:0.58rem;font-weight:800;display:${window.notifCount > 0 ? 'flex' : 'none'};align-items:center;justify-content:center;border:2px solid #07070f;padding:0 2px;line-height:1;">${window.notifCount || ''}</span>` : ''}
            <span style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;">${it.label}</span>
        </button>
        `).join('')}
        <button onclick="logout()" style="background:none;border:none;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:4px;padding:6px 14px;border-radius:14px;color:rgba(255,99,99,0.7);">
            <i class="fas fa-sign-out-alt" style="font-size:1.3rem;"></i>
            <span style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;">Salir</span>
        </button>
    </nav>
...
async function render(view) {
    ...
    // ── DASHBOARD ──
        ...
        app.innerHTML = `
            ...
            `}
            <div style="max-width:${IS_MOBILE ? '100%' : '1400px'};margin:auto;padding:${IS_MOBILE ? '16px 12px' : '32px 24px'};">
                ...
                <div class="${IS_MOBILE ? '' : 'grid'}" style="${IS_MOBILE ? 'display:grid;grid-template-columns:repeat(2,1fr);gap:10px;padding:14px 0;' : ''}">
                    ${feedItems.map((item, i) => {
                        if(item.itemType === 'job') {
                            ...
                            if (IS_MOBILE) return `
                            <div class="glass" data-id="${item.id}" style="border-radius:18px;padding:12px 10px;display:flex;flex-direction:column;align-items:center;text-align:center;gap:8px;animation-delay:${i*0.05}s;border:1px solid rgba(10,132,255,0.12);position:relative;overflow:hidden;">
                                <span style="position:absolute;top:8px;left:8px;background:${isDisponible ? 'rgba(10,132,255,0.18)' : 'rgba(255,59,48,0.15)'};color:${isDisponible ? '#0a84ff' : '#ff6b63'};font-size:0.55rem;font-weight:800;padding:3px 7px;border-radius:20px;text-transform:uppercase;letter-spacing:0.5px;">${isDisponible ? '● Activo' : '● Cerrado'}</span>
                                ${hasEmpFoto
                                    ? `<img src="${empresaFoto}" style="width:54px;height:54px;border-radius:50%;object-fit:cover;border:2px solid rgba(10,132,255,0.3);margin-top:14px;">`
                                    : `<div style="width:54px;height:54px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:rgba(10,132,255,0.1);color:#0a84ff;font-size:1.3rem;border:2px solid rgba(10,132,255,0.2);margin-top:14px;"><i class="fas fa-building"></i></div>`
                                }
                                <div style="font-size:0.72rem;font-weight:900;color:#fff;text-transform:uppercase;letter-spacing:0.5px;line-height:1.2;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${item.empresa_nombre || 'Empresa'}</div>
                                <div style="font-size:0.65rem;color:#0a84ff;font-weight:700;text-transform:uppercase;letter-spacing:0.3px;line-height:1.2;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${item.titulo || 'Puesto'}</div>
                                <div style="font-size:0.6rem;color:rgba(255,255,255,0.4);"><i class="fas fa-map-marker-alt" style="color:#0a84ff;"></i> ${(item.ubicacion || 'Sin ubicación').substring(0,18)}</div>
                                <div style="height:1px;width:100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.08),transparent);"></div>
                                <div style="font-size:1.1rem;font-weight:900;color:#fff;letter-spacing:-0.5px;">${fmtSal(item.salario)}</div>
                                <button onclick="openJobDetails(${item.id})" class="btn-premium" style="width:100%;padding:9px 6px;font-size:0.7rem;border-radius:12px;justify-content:center;margin-top:2px;">
                                    <i class="fas fa-paper-plane"></i> Postularme
                                </button>
                                ${isOwner ? `<button onclick="toggleJobStatus(${item.id})" style="background:none;border:none;color:rgba(255,255,255,0.3);font-size:0.58rem;cursor:pointer;padding:2px;">${isDisponible ? 'Cerrar vacante' : 'Reactivar'}</button>` : ''}
                            </div>`;
                            ...
                            if (IS_MOBILE) return `
                            <div class="glass" data-candidate-id="${item.id}" style="border-radius:18px;padding:12px 10px;display:flex;flex-direction:column;align-items:center;text-align:center;gap:8px;animation-delay:${i*0.05}s;border:1px solid rgba(255,215,0,0.12);position:relative;overflow:hidden;">
                                <span style="position:absolute;top:8px;left:8px;background:${isCandDisponible ? 'rgba(48,209,88,0.18)' : 'rgba(255,59,48,0.15)'};color:${isCandDisponible ? '#30d158' : '#ff6b63'};font-size:0.55rem;font-weight:800;padding:3px 7px;border-radius:20px;text-transform:uppercase;letter-spacing:0.5px;">${isCandDisponible ? '● Libre' : '● Ocupado'}</span>
                                ${hasCFoto
                                    ? `<img src="${cFoto}" style="width:54px;height:54px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,215,0,0.3);margin-top:14px;">`
                                    : `<div style="width:54px;height:54px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:rgba(255,215,0,0.08);color:#FFD700;font-size:1.3rem;border:2px solid rgba(255,215,0,0.2);margin-top:14px;"><i class="fas fa-user"></i></div>`
                                }
                                <div style="font-size:0.72rem;font-weight:900;color:#fff;text-transform:uppercase;letter-spacing:0.5px;line-height:1.2;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${item.nombre || 'Candidato'}</div>
                                <div style="font-size:0.65rem;color:#FFD700;font-weight:700;text-transform:uppercase;letter-spacing:0.3px;line-height:1.2;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${item.profesion || 'Profesional'}</div>
                                <div style="font-size:0.6rem;color:rgba(255,255,255,0.4);"><i class="fas fa-map-marker-alt" style="color:#FFD700;"></i> ${((cv.ubicacion || 'Sin ubicación')).substring(0,18)}</div>
                                <div style="height:1px;width:100%;background:linear-gradient(90deg,transparent,rgba(255,215,0,0.1),transparent);"></div>
                                <div style="font-size:1.1rem;font-weight:900;color:#fff;letter-spacing:-0.5px;">${cv.moneda || 'Q'} ${fmtSal(item.salario_deseado)}</div>
                                <button onclick="openCandidateDetails(${item.id})" class="btn-premium btn-yellow-gradient" style="width:100%;padding:9px 6px;font-size:0.7rem;border-radius:12px;justify-content:center;margin-top:2px;">
                                    <i class="fas fa-user-check"></i> Ver Talento
                                </button>
                                ${isOwner ? `<button onclick="toggleCandidateStatus(${item.id})" style="background:none;border:none;color:rgba(255,255,255,0.3);font-size:0.58rem;cursor:pointer;padding:2px;">${isCandDisponible ? 'Marcar ocupado' : 'Marcar libre'}</button>` : ''}
                            </div>`;
                ...
                <div style="max-width:1100px;margin:auto;padding:${IS_MOBILE ? '24px 16px' : '40px 32px'};">
                    ...
                    ` : empleosConPostulantes.map(empleo => `
                        <div style="margin-bottom:40px;">
                            ...
                            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:18px;padding-bottom:16px;border-bottom:1px solid rgba(255,255,255,0.06);">
                                <div style="display:flex;align-items:center;gap:14px;">
                                    <div style="width:46px;height:46px;border-radius:14px;background:rgba(10,132,255,0.1);border:1px solid rgba(10,132,255,0.2);display:flex;align-items:center;justify-content:center;">
                                        <i class="fas fa-briefcase" style="color:#0a84ff;font-size:1.2rem;"></i>
                                    </div>
                                    <div>
                                        <div style="font-size:1.15rem;font-weight:900;color:#fff;text-transform:uppercase;letter-spacing:0.5px;">${empleo.titulo}</div>
                                        <div style="display:flex;gap:14px;margin-top:4px;flex-wrap:wrap;">
                                            <span style="font-size:0.72rem;color:rgba(255,255,255,0.4);"><i class="fas fa-map-marker-alt" style="color:#0a84ff;margin-right:5px;"></i>${empleo.ubicacion || 'No especificada'}</span>
                                            <span style="font-size:0.72rem;color:rgba(255,255,255,0.4);"><i class="fas fa-laptop-house" style="color:#0a84ff;margin-right:5px;"></i>${empleo.modalidad || 'Presencial'}</span>
                                            <span style="font-size:0.72rem;color:rgba(255,255,255,0.4);"><i class="fas fa-coins" style="color:#0a84ff;margin-right:5px;"></i>${fmtSal(empleo.salario)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:16px;">
                                ${empleo.postulantes.map(p => {
                                    ...
                                    const skills = (cv.skills || '').split(',').map(s => s.trim()).filter(s => s).slice(0, 4);
                                    return `
                                    <div class="glass" style="border-radius:20px;border:1px solid rgba(255,255,255,0.07);overflow:hidden;display:flex;flex-direction:column;transition:0.2s;width:320px;flex-shrink:0;">
                                        <!-- Cabecera degradado -->
                                        <div style="background:linear-gradient(135deg,rgba(10,132,255,0.1),rgba(10,132,255,0.03));padding:24px 20px 18px;text-align:center;border-bottom:1px solid rgba(255,255,255,0.05);">
                                            ${hasFoto
                                                ? `<img src="${fotoUrl}" style="width:72px;height:72px;border-radius:50%;object-fit:cover;border:3px solid rgba(10,132,255,0.3);box-shadow:0 0 0 5px rgba(10,132,255,0.07);margin-bottom:12px;">`
                                                : `<div style="width:72px;height:72px;border-radius:50%;background:rgba(10,132,255,0.08);display:flex;align-items:center;justify-content:center;color:#0a84ff;font-size:1.9rem;border:3px solid rgba(10,132,255,0.25);box-shadow:0 0 0 5px rgba(10,132,255,0.06);margin:0 auto 12px;"><i class="fas fa-user"></i></div>`
                                            }
                                            <div style="font-size:1rem;font-weight:800;color:#fff;text-transform:uppercase;letter-spacing:0.5px;">${p.nombre}</div>
                                            <div style="font-size:0.72rem;color:#0a84ff;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-top:3px;">${p.profesion || 'Profesional'}</div>
                                        </div>
                                        <!-- Contenido -->
                                        <div style="padding:16px 18px;flex:1;display:flex;flex-direction:column;gap:10px;">
                                            ${cv.bio ? `<p style="font-size:0.82rem;color:rgba(255,255,255,0.55);line-height:1.65;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;margin:0;">${cv.bio}</p>` : ''}
                                            ${skills.length > 0 ? `<div style="display:flex;flex-wrap:wrap;gap:5px;">${skills.map(s => `<span style="background:rgba(10,132,255,0.1);color:#0a84ff;padding:3px 10px;border-radius:50px;font-size:0.68rem;font-weight:700;border:1px solid rgba(10,132,255,0.2);">${s}</span>`).join('')}</div>` : ''}
                                            ${cv.ubicacion ? `<div style="font-size:0.75rem;color:rgba(255,255,255,0.35);"><i class="fas fa-map-marker-alt" style="color:#0a84ff;margin-right:5px;"></i>${cv.ubicacion}</div>` : ''}
                                            <div style="font-size:0.7rem;color:rgba(255,255,255,0.25);margin-top:auto;"><i class="fas fa-calendar-alt" style="margin-right:4px;"></i>Aplicó el ${new Date(p.fecha_postulacion).toLocaleDateString('es-GT',{day:'numeric',month:'long',year:'numeric'})}</div>
                                        </div>
                                        <!-- Botones -->
                                        <div style="padding:0 16px 16px;display:flex;flex-direction:column;gap:8px;">
                                            <button onclick="abrirPerfilPostulante(${p.id})" class="btn-premium" style="width:100%;padding:12px;font-size:0.83rem;justify-content:center;background:linear-gradient(135deg,#0a84ff,#0056b3);box-shadow:0 6px 18px rgba(10,132,255,0.3);">
                                                <i class="fas fa-user-check"></i> Ver Perfil Completo
                                            </button>
                                            ${p.estado === 'aceptado' ? `
                                            <div style="text-align:center;padding:8px;border-radius:10px;background:rgba(48,209,88,0.1);border:1px solid rgba(48,209,88,0.25);font-size:0.75rem;font-weight:700;color:#30d158;"><i class="fas fa-check-circle" style="margin-right:5px;"></i>Ya aceptado</div>
                                            ` : p.estado === 'rechazado' ? `
                                            <div style="text-align:center;padding:8px;border-radius:10px;background:rgba(255,59,48,0.1);border:1px solid rgba(255,59,48,0.25);font-size:0.75rem;font-weight:700;color:#ff6b63;"><i class="fas fa-times-circle" style="margin-right:5px;"></i>Ya rechazado</div>
                                            ` : `
                                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                                                <button onclick="rechazarPostulante(${p.postulacion_id},window.postulantesCache[${p.id}]?.nombre||'')" style="padding:10px;border-radius:12px;background:rgba(255,59,48,0.12);border:1px solid rgba(255,59,48,0.3);color:#ff6b63;font-weight:800;font-size:0.78rem;cursor:pointer;font-family:inherit;transition:0.2s;">
                                                    <i class="fas fa-times"></i> Rechazar
                                                </button>
                                                <button onclick="aceptarPostulante(${p.postulacion_id},window.postulantesCache[${p.id}]?.nombre||'')" style="padding:10px;border-radius:12px;background:rgba(48,209,88,0.12);border:1px solid rgba(48,209,88,0.3);color:#30d158;font-weight:800;font-size:0.78rem;cursor:pointer;font-family:inherit;transition:0.2s;">
