...
async function aceptarPostulante(postulacion_id, nombre) {
    ...
    try {
        ...
        if (!r.ok) { notify('Error', data.error || `Error ${r.status}`, 'error'); return; }
        notify('¡Aceptado!', `${nombre} ha sido aceptado.`);
        render('misPostulacionesEmpresa');
    } catch(e) {
