...
function getJobStatus(id) {
    const local = localStorage.getItem(`jobStatus_${id}`);
...
async function toggleJobStatus(id) {
    ...
    const item = window.feedItems?.find(j => j.id === id && j.itemType === 'job');
    if (item) item.disponible = newStatus === 'disponible';
    try {
        await fetch(`${API_URL}/empleos/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
            body: JSON.stringify({ disponible: newStatus === 'disponible' })
        });
    } catch(e) {}
}

function getCandidateStatus(id) {
    const local = localStorage.getItem(`candidateStatus_${id}`);
    if (local) return local;
