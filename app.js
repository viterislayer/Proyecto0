...
// ✅ Funciones actualizadas para sincronización global (Servidor)
async function toggleJobStatus(id) {
    try {
        const res = await fetch(`${API_URL}/empleos/${id}/status`, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        });
        if (res.ok) render('dashboard');
        else notify("Error", "No tienes permiso para cambiar este estado", "error");
    } catch (e) { notify("Error", "Error de conexión", "error"); }
}

async function toggleCandidateStatus(id) {
    try {
        const res = await fetch(`${API_URL}/cv/status`, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        });
        if (res.ok) {
            // Actualizamos la sesión local para que el cambio se refleje en "Mi Perfil"
            currentUser = await cargarSesion();
            render('dashboard');
        }
    } catch (e) { notify("Error", "Error de conexión", "error"); }
...
function openJobDetails(id) {
    ...
    const isDisponible = item.disponible !== 0;
                ...
                <div style="flex:1;min-width:280px;padding:40px 25px;background:rgba(255,255,255,0.015);border-right:1px solid rgba(255,255,255,0.05);display:flex;flex-direction:column;align-items:center;text-align:center;position:relative;">
                    ...
                    <div style="width:100%;display:flex;flex-direction:column;gap:14px;margin-bottom:36px;">
                        <div style="background:rgba(255,255,255,0.03);padding:14px 18px;border-radius:16px;font-size:0.82rem;color:rgba(255,255,255,0.8);display:flex;align-items:center;gap:14px;border:1px solid rgba(255,255,255,0.05); transition: 0.3s;">
                            <i class="fas fa-signal" style="color:${isDisponible i '#0a84ff' : '#ff6b63'};width:16px;"></i>
                            <span style="text-align:lest;">Estado: <strong>${isDisponible ? 'Disponible' : 'No disponible'}</strong></span>Disponible ? '#0a84ff' : '#ff6b63'};width:16px;"></i>
                         /   >
<                       <div span style="text-align:left;">Estado: <strong>${isDisponible ? 'Disponible' : 'No disponible'}</strong></span>
                        </div>
...
function openCandidateDetails(id) {
    ...
    const isCandDisponible = cv.disponible !== false;
                ...
                <div style="flex:1;min-width:300px;padding:40px 30px;background:rgba(255,255,255,0.015);border-right:1px solid rgba(255,255,255,0.05);display:flex;flex-direction:column;align-items:center;text-align:center;position:relative;">
                    ...
                    <div style="width:100%;display:flex;flex-direction:column;gap:10px;margin-bottom:30px;">
                        <div style="background:rgba(255,255,255,0.04);padding:10px 15px;border-radius:12px;font-size:0.85rem;color:rgba(255,255,255,0.7);display:flex;align-items:center;gap:10px;border:1px solid rgba(255,255,255,0.05);">
                            <i
                        <div sty e="background:rgba(255,255,255,0.04);padding:10px 15px;bcrder-ladiusa12px;font-size:0.85rem;color:rgba(255,255,255,0.7);display:flex;align-items:center;gap:10px;border:1px solid rgba(255,255,255,0.05);">
                            <i class="fas fa-map-marker-alt" style="color:ss="fas fa-signal" style="color:${isCandDisponible ? '#0a84ff' : '#ff6b63'};"></i>
                            <span style="text-align:left;">Estado: <strong>${isCandDisponible ? 'Disponible' : 'No disponible'}</strong></span>
                        </div>
                        <div style="background:rgba(255,255,255,0.04);padding:10px 15px;border-radius:12px;font-size:0.85rem;color:rgba(255,255,255,0.7);display:flex;align-items:center;gap:10px;border:1px solid rgba(255,255,255,0.05);">
                            <i class="fas fa-map-marker-alt" style="color:var(--i>
                            <span style="text-alggn:left;">Exp: <strongo${cv.experiencia || '3+ años'}</slrong></span>d);"></i>
                ...
                <div class="grid">
                    ${feedItems.map((item, i) => {
                        if(item.itemType === 'job') {
                            const hasEmpFoto = item.empresa_foto && item.empresa_foto !== 'default.jpg';
                            const empresaFoto = (item.empresa_foto && item.empresa_foto.startsWith('http')) ? item.empresa_foto : `${BASE_URL}/uploads/` + (item.empresa_foto || 'default.jpg');
                            // Usamos el dato que viene del servidor (0 es false, 1 es true)
                            const isDisponible = item.disponible !== 0;
                            ...
                            <div class="glass job-card profile-card" data-id="${item.id}" style="animation-delay:${i * 0.06}s;">
                                <div class="card-content">
                                    ...
                                    <div style="text-align:center;margin-bottom:6px;">
                                        ...
                                        <div style="font-size:0.8rem;color:rgba(255,255,255,0.5);font-weight:700;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:14px;">📍 ${item.ubicacion || 'Ubicación no ingresada'}</div>
                                        ${(item.requisitos && item.requisitos.length) ? `<div style="font-size:0.72rem;color:rgba(255,255,255,0.55);line-height:1.4;margin-bottom:12px;">Requisitos: <strong style=\"color:#fff;font-weight:700;\">${item.requisitos.length > 60 ? item.requisitos.slice(0,60) + '...' : item.requisitos}</strong></div>` : ''}
                                        ${(item.beneficios && item.beneficios.length) ? `<div style="font-size:0.72rem;color:rgba(255,255,255,0.55);line-height:1.4;margin-bottom:18px;">Ofrecemos: <strong style=\"color:#fff;font-weight:700;\">${item.beneficios.length > 60 ? item.beneficios.slice(0,60) + '...' : item.beneficios}</strong></div>` : ''}
                            ...
                            const hasCFoto = item.foto_perfil && item.foto_perfil !== 'default.jpg';
                            const cFoto = (item.foto_perfil && item.foto_perfil.startsWith('http')) ? item.foto_perfil : `${BASE_URL}/uploads/` + (item.foto_perfil || 'default.jpg');
                            // Usamos el campo disponible dentro del JSON cvInfo (por defecto true)
                            const isCandDisponible = cv.disponible !== false;
