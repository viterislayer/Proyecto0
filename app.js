...
async function cambiarEstadoPostulante(postulacion_id, nombre, estado, btn) {
    const pid = postulacion_id || btn.closest('[data-pid]')?.dataset?.pid;
    if (!pid || pid === 'undefined') { notify('Error', 'No se pudo identificar la postulación. Recarga la página.', 'error'); return; }
    ...
    try {
        const r = await fetch(`${API_URL}/postulaciones/${pid}/estado`, {
            method: 'POST',
    ...
    // ── POSTULACIONES EMPRESA ──
        ...
        try {
            ...
            app.innerHTML = `
                ...
                <!-- HERO HEADER -->
                <div style="position:relative;overflow:hidden;background:linear-gradient(135deg,rgba(10,132,255,0.1) 0%,rgba(0,0,0,0) 55%),linear-gradient(225deg,rgba(48,209,88,0.06) 0%,transparent 50%);border-bottom:1px solid rgba(255,255,255,0.06);padding:${IS_MOBILE?'36px 20px 30px':'52px 48px 42px'};text-align:center;">
                    <div style="position:absolute;top:-60px;left:50%;transform:translateX(-50%);width:300px;height:300px;background:radial-gradient(circle,rgba(10,132,255,0.08),transparent 70%);pointer-events:none;"></div>
                    <div style="position:relative;">
                        <div style="display:inline-flex;align-items:center;gap:10px;background:rgba(10,132,255,0.1);border:1px solid rgba(10,132,255,0.25);border-radius:50px;padding:7px 18px;margin-bottom:20px;">
                            <i class="fas fa-circle" style="color:#0a84ff;font-size:0.4rem;"></i>
                            <span style="font-size:0.7rem;font-weight:700;color:#0a84ff;text-transform:uppercase;letter-spacing:1.5px;">Panel de Reclutamiento</span>
                        </div>
                        <h1 style="font-size:${IS_MOBILE?'1.9rem':'2.6rem'};font-weight:900;color:#fff;letter-spacing:-1.5px;margin-bottom:10px;line-height:1.1;">Candidatos <span style="background:linear-gradient(135deg,#0a84ff,#30d158);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">Recibidos</span></h1>
                        <p style="color:rgba(255,255,255,0.4);font-size:0.92rem;margin-bottom:28px;max-width:440px;margin-left:auto;margin-right:auto;line-height:1.5;">Personas interesadas en unirse a tu equipo</p>
                        <div style="display:inline-flex;gap:0;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:20px;overflow:hidden;">
                            <div style="text-align:center;padding:16px 32px;">
                                <div style="font-size:${IS_MOBILE?'1.8rem':'2.2rem'};font-weight:900;color:#0a84ff;line-height:1;letter-spacing:-1px;">${totalPost}</div>
                                <div style="font-size:0.65rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:1.2px;font-weight:700;margin-top:5px;">Postulante${totalPost!==1?'s':''}</div>
                            </div>
                            <div style="width:1px;background:rgba(255,255,255,0.07);margin:12px 0;"></div>
                            <div style="text-align:center;padding:16px 32px;">
                                <div style="font-size:${IS_MOBILE?'1.8rem':'2.2rem'};font-weight:900;color:#30d158;line-height:1;letter-spacing:-1px;">${empleosConPostulantes.length}</div>
                                <div style="font-size:0.65rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:1.2px;font-weight:700;margin-top:5px;">Anuncio${empleosConPostulantes.length!==1?'s':''}</div>
                            </div>
                            <div style="width:1px;background:rgba(255,255,255,0.07);margin:12px 0;"></div>
                            <div style="text-align:center;padding:16px 32px;">
                                <div style="font-size:${IS_MOBILE?'1.8rem':'2.2rem'};font-weight:900;color:#ffd60a;line-height:1;letter-spacing:-1px;">${empleosConPostulantes.reduce((s,e)=>s+e.postulantes.filter(x=>!x.estado||x.estado==='pendiente').length,0)}</div>
                                <div style="font-size:0.65rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:1.2px;font-weight:700;margin-top:5px;">Pendientes</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div style="max-width:1100px;margin:auto;padding:${IS_MOBILE ? '24px 16px' : '40px 32px'};">
                    ...
                    ` : empleosConPostulantes.map(empleo => `
                        <div style="margin-bottom:52px;">
                            ...
                            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px;padding:${IS_MOBILE?'16px 18px':'22px 28px'};background:linear-gradient(135deg,rgba(10,132,255,0.1),rgba(10,132,255,0.02));border:1px solid rgba(10,132,255,0.22);border-left:5px solid #0a84ff;border-radius:18px;margin-bottom:14px;box-shadow:0 8px 32px rgba(10,132,255,0.08);">
                                <div style="display:flex;align-items:center;gap:16px;">
                                    <div style="width:${IS_MOBILE?'40px':'52px'};height:${IS_MOBILE?'40px':'52px'};border-radius:15px;background:linear-gradient(135deg,rgba(10,132,255,0.25),rgba(10,132,255,0.1));border:1px solid rgba(10,132,255,0.35);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 0 20px rgba(10,132,255,0.2);">
                                        <i class="fas fa-briefcase" style="color:#0a84ff;font-size:${IS_MOBILE?'1.1rem':'1.3rem'};"></i>
                                    </div>
                                    <div>
                                        <div style="font-size:${IS_MOBILE?'1rem':'1.15rem'};font-weight:900;color:#fff;text-transform:uppercase;letter-spacing:1.2px;margin-bottom:5px;text-shadow:0 0 20px rgba(255,255,255,0.1);">${empleo.titulo}</div>
                                        <div style="display:flex;gap:16px;flex-wrap:wrap;">
                                            <span style="font-size:0.69rem;color:rgba(255,255,255,0.45);display:flex;align-items:center;gap:5px;"><i class="fas fa-map-marker-alt" style="color:#0a84ff;"></i>${empleo.ubicacion||'Sin ubicación'}</span>
                                            <span style="font-size:0.69rem;color:rgba(255,255,255,0.45);display:flex;align-items:center;gap:5px;"><i class="fas fa-laptop-house" style="color:#0a84ff;"></i>${empleo.modalidad||'Presencial'}</span>
                                            <span style="font-size:0.69rem;color:rgba(255,255,255,0.45);display:flex;align-items:center;gap:5px;"><i class="fas fa-coins" style="color:#0a84ff;"></i>${fmtSal(empleo.salario)}</span>
                                        </div>
                                    </div>
                                </div>
                                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                                    <span style="background:rgba(255,214,10,0.1);color:#ffd60a;padding:5px 13px;border-radius:50px;font-size:0.67rem;font-weight:700;border:1px solid rgba(255,214,10,0.25);display:flex;align-items:center;gap:5px;"><i class="fas fa-clock"></i>${empleo.postulantes.filter(x=>!x.estado||x.estado==='pendiente').length} pendientes</span>
                                    <span style="background:rgba(48,209,88,0.1);color:#30d158;padding:5px 13px;border-radius:50px;font-size:0.67rem;font-weight:700;border:1px solid rgba(48,209,88,0.25);display:flex;align-items:center;gap:5px;"><i class="fas fa-check"></i>${empleo.postulantes.filter(x=>x.estado==='aceptado').length} aceptados</span>
                                    <span style="background:rgba(255,59,48,0.1);color:#ff6b63;padding:5px 13px;border-radius:50px;font-size:0.67rem;font-weight:700;border:1px solid rgba(255,59,48,0.25);display:flex;align-items:center;gap:5px;"><i class="fas fa-times"></i>${empleo.postulantes.filter(x=>x.estado==='rechazado').length} rechazados</span>
                                </div>
                            </div>

                            <!-- Lista de candidatos -->
                            <div style="display:flex;flex-direction:column;gap:10px;">
                                ${empleo.postulantes.map((p, pi) => {
                                    let cv = {};
                                    ...
                                    const accentBg = p.estado === 'aceptado' ? 'rgba(48,209,88,0.06)' : p.estado === 'rechazado' ? 'rgba(255,59,48,0.05)' : 'rgba(255,214,10,0.04)';
                                    const dateStr = new Date(p.fecha_postulacion).toLocaleDateString('es-GT',{day:'numeric',month:'long',year:'numeric'});
                                    const rank = String(pi + 1).padStart(2, '0');
                                    const safePid = p.postulacion_id || ((empleo.id || p.empleo_id || '0') + '_' + p.id);
                                    return `
                                    <div data-pid="${safePid}" data-pnombre="${p.nombre}" style="display:flex;align-items:center;gap:${IS_MOBILE?'10px':'16px'};padding:${IS_MOBILE?'14px 16px':'20px 26px'};background:linear-gradient(135deg,${accentBg},rgba(255,255,255,0.015));border:1px solid rgba(255,255,255,0.08);border-radius:18px;position:relative;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.12);">
                                        <div style="position:absolute;left:0;top:0;bottom:0;width:4px;background:linear-gradient(180deg,${accentColor},${accentColor}55);border-radius:4px 0 0 4px;box-shadow:0 0 8px ${accentColor}44;"></div>

                                        ${IS_MOBILE ? '' : `<div style="flex-shrink:0;width:26px;text-align:center;margin-left:4px;">
                                            <span style="font-size:0.65rem;font-weight:900;color:rgba(255,255,255,0.15);letter-spacing:-0.5px;font-variant-numeric:tabular-nums;">#${rank}</span>
                                        </div>`}

                                        <div style="flex-shrink:0;${IS_MOBILE?'margin-left:6px;':''}">
                                            ${hasFoto
                                                ? `<img src="${fotoUrl}" style="width:${IS_MOBILE?'46px':'58px'};height:${IS_MOBILE?'46px':'58px'};border-radius:50%;object-fit:cover;border:2.5px solid ${accentColor}66;box-shadow:0 0 0 4px ${accentColor}18,0 6px 16px rgba(0,0,0,0.3);">`
                                                : `<div style="width:${IS_MOBILE?'46px':'58px'};height:${IS_MOBILE?'46px':'58px'};border-radius:50%;background:linear-gradient(135deg,rgba(10,132,255,0.18),rgba(10,132,255,0.06));display:flex;align-items:center;justify-content:center;color:#0a84ff;font-size:${IS_MOBILE?'1.2rem':'1.5rem'};border:2.5px solid rgba(10,132,255,0.28);box-shadow:0 0 0 4px rgba(10,132,255,0.08);"><i class="fas fa-user"></i></div>`
                                            }
                                        </div>

                                        <div style="flex:1;min-width:0;">
                                            <div style="font-size:${IS_MOBILE?'0.9rem':'1.02rem'};font-weight:900;color:#fff;letter-spacing:0.2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:2px;">${p.nombre}</div>
                                            <div style="font-size:0.68rem;color:#0a84ff;font-weight:700;text-transform:uppercase;letter-spacing:0.9px;">${p.profesion||'Profesional'}</div>
                                            <div style="font-size:0.62rem;color:rgba(255,255,255,0.2);margin-top:5px;display:flex;align-items:center;gap:4px;">
                                                <i class="fas fa-calendar-alt" style="color:rgba(255,255,255,0.15);"></i>${dateStr}
                                            </div>
                                            ${skills.length > 0 && !IS_MOBILE ? `<div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:7px;">${skills.map(s=>`<span style="background:rgba(10,132,255,0.1);color:rgba(10,132,255,0.75);padding:2px 8px;border-radius:50px;font-size:0.6rem;font-weight:700;border:1px solid rgba(10,132,255,0.18);">${s}</span>`).join('')}</div>` : ''}
                                        </div>

                                        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:10px;flex-shrink:0;">
                                            ${p.estado === 'aceptado'
                                                ? `<span style="background:linear-gradient(135deg,rgba(48,209,88,0.15),rgba(48,209,88,0.06));color:#30d158;padding:5px 14px;border-radius:50px;font-size:0.63rem;font-weight:800;border:1px solid rgba(48,209,88,0.3);text-transform:uppercase;letter-spacing:0.7px;white-space:nowrap;box-shadow:0 0 14px rgba(48,209,88,0.15);"><i class="fas fa-check-circle" style="margin-right:4px;"></i>Aceptado</span>`
                                                : p.estado === 'rechazado'
                                                ? `<span style="background:rgba(255,59,48,0.1);color:#ff6b63;padding:5px 14px;border-radius:50px;font-size:0.63rem;font-weight:800;border:1px solid rgba(255,59,48,0.28);text-transform:uppercase;letter-spacing:0.7px;white-space:nowrap;"><i class="fas fa-times-circle" style="margin-right:4px;"></i>Rechazado</span>`
                                                : `<span style="background:rgba(255,214,10,0.1);color:#ffd60a;padding:5px 14px;border-radius:50px;font-size:0.63rem;font-weight:800;border:1px solid rgba(255,214,10,0.25);text-transform:uppercase;letter-spacing:0.7px;white-space:nowrap;"><i class="fas fa-clock" style="margin-right:4px;"></i>Pendiente</span>`
                                            }
                                            <div style="display:flex;gap:7px;align-items:center;">
                                                <button onclick="abrirPerfilPostulante(${p.id})" style="padding:${IS_MOBILE?'8px 11px':'8px 16px'};border-radius:10px;background:rgba(10,132,255,0.1);border:1px solid rgba(10,132,255,0.25);color:#0a84ff;font-weight:700;font-size:0.7rem;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:6px;white-space:nowrap;">
                                                    <i class="fas fa-eye"></i>${IS_MOBILE?'':' Perfil'}
                                                </button>
                                                <div data-acciones style="display:flex;gap:7px;">
                                                    ${p.estado === 'aceptado' ? `
                                                        <div style="padding:8px 16px;border-radius:10px;background:rgba(48,209,88,0.08);border:1px solid rgba(48,209,88,0.25);color:#30d158;font-size:0.7rem;font-weight:700;display:flex;align-items:center;gap:5px;white-space:nowrap;">
                                                            <i class="fas fa-check-circle"></i>${IS_MOBILE?'':' Aceptado'}
                                                        </div>
                                                    ` : p.estado === 'rechazado' ? `
                                                        <div style="padding:8px 16px;border-radius:10px;background:rgba(255,59,48,0.08);border:1px solid rgba(255,59,48,0.25);color:#ff6b63;font-size:0.7rem;font-weight:700;display:flex;align-items:center;gap:5px;white-space:nowrap;">
                                                            <i class="fas fa-times-circle"></i>${IS_MOBILE?'':' Rechazado'}
                                                        </div>
                                                    ` : `
                                                        <button onclick="cambiarEstadoPostulante(this.closest('[data-pid]').dataset.pid,this.closest('[data-pid]').dataset.pnombre||'Candidato','aceptado',this)" style="padding:${IS_MOBILE?'8px 12px':'9px 18px'};border-radius:10px;background:linear-gradient(135deg,rgba(48,209,88,0.2),rgba(48,209,88,0.08));border:1.5px solid rgba(48,209,88,0.35);color:#30d158;font-weight:800;font-size:0.72rem;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:6px;white-space:nowrap;box-shadow:0 4px 12px rgba(48,209,88,0.15);">
                                                            <i class="fas fa-user-check"></i>${IS_MOBILE?'':' Aceptar'}
                                                        </button>
                                                        <button onclick="cambiarEstadoPostulante(this.closest('[data-pid]').dataset.pid,this.closest('[data-pid]').dataset.pnombre||'Candidato','rechazado',this)" style="padding:${IS_MOBILE?'8px 12px':'9px 16px'};border-radius:10px;background:rgba(255,59,48,0.08);border:1px solid rgba(255,59,48,0.25);color:#ff6b63;font-weight:700;font-size:0.72rem;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:6px;white-space:nowrap;">
                                                            <i class="fas fa-user-times"></i>${IS_MOBILE?'':' Rechazar'}
                                                        </button>
                                                    `}
                                                </div>
                                            </div>
