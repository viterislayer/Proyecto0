...
async function cambiarEstadoPostulante(postulacion_id, nombre, estado, btn) {
    const pid = Number(postulacion_id) || Number(btn.closest('[data-pid]')?.dataset?.pid);
    if (!pid) { notify('Error', 'Recarga la página e inténtalo de nuevo.', 'error'); return; }
                        ...
                        <div style="margin-bottom:44px;">
                            ...
                            <div style="display:flex;flex-direction:column;gap:9px;">
                                ${empleo.postulantes.map(p => {
                                    ...
                                    return `
                                    <div data-pid="${p.postulacion_id}" data-pnombre="${p.nombre}" style="display:flex;align-items:center;gap:${IS_MOBILE?'11px':'18px'};padding:${IS_MOBILE?'13px 14px':'16px 22px'};background:rgba(255,255,255,0.024);border:1px solid rgba(255,255,255,0.07);border-radius:15px;position:relative;overflow:hidden;transition:border-color 0.2s;">
                                        ...
                                        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px;flex-shrink:0;">
                                            ...
                                            <div style="display:flex;gap:6px;align-items:center;">
                                                ...
                                                <div data-acciones style="display:flex;gap:6px;">
                                                    ...
                                                    ` : `
                                                        <button onclick="cambiarEstadoPostulante(this.closest('[data-pid]').dataset.pid,this.closest('[data-pid]').dataset.pnombre||'Candidato','aceptado',this)" style="padding:${IS_MOBILE?'7px 10px':'7px 13px'};border-radius:9px;background:rgba(48,209,88,0.12);border:1.5px solid rgba(48,209,88,0.3);color:#30d158;font-weight:800;font-size:0.7rem;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:5px;white-space:nowrap;transition:background 0.15s;"><i class="fas fa-user-check"></i>${IS_MOBILE?'':' Aceptar'}</button>
                                                        <button onclick="cambiarEstadoPostulante(this.closest('[data-pid]').dataset.pid,this.closest('[data-pid]').dataset.pnombre||'Candidato','rechazado',this)" style="padding:${IS_MOBILE?'7px 10px':'7px 13px'};border-radius:9px;background:rgba(255,59,48,0.07);border:1px solid rgba(255,59,48,0.22);color:#ff6b63;font-weight:700;font-size:0.7rem;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:5px;white-space:nowrap;transition:background 0.15s;"><i class="fas fa-user-times"></i>${IS_MOBILE?'':' Rechazar'}</button>
                                                    `}
                                                </div>
                                            </div>
                                        </div>
