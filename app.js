                ...
                <div class="${IS_MOBILE ? '' : 'grid'}" style="${IS_MOBILE ? 'display:grid;grid-template-columns:repeat(2,1fr);gap:10px;padding:14px 0 84px;grid-auto-rows:330px;' : ''}">
                    ${feedItems.map((item, i) => {
                        if(item.itemType === 'job') {
                            ...
                            if (IS_MOBILE) return `
                            <div class="glass" data-id="${item.id}" style="border-radius:20px;padding:14px 12px 60px;display:flex;flex-direction:column;align-items:center;text-align:center;gap:10px;animation-delay:${i*0.05}s;border:1px solid rgba(10,132,255,0.2);position:relative;overflow:hidden;height:330px;box-sizing:border-box;">
                                <span style="align-self:flex-start;background:${isDisponible?'rgba(10,132,255,0.18)':'rgba(255,59,48,0.15)'};color:${isDisponible?'#0a84ff':'#ff6b63'};font-size:0.62rem;font-weight:800;padding:4px 10px;border-radius:20px;text-transform:uppercase;letter-spacing:0.5px;">${isDisponible?'● Libre':'● Ocupado'}</span>
                                ${isOwner ? `<button onclick="eliminarAnuncio(${item.id})" style="position:absolute;top:10px;right:10px;background:rgba(255,59,48,0.12);border:1px solid rgba(255,59,48,0.25);color:#ff6b63;border-radius:8px;padding:4px 7px;font-size:0.58rem;cursor:pointer;z-index:10;"><i class="fas fa-trash-alt"></i></button>` : ''}
                                ${hasEmpFoto
                                ...
                                <div style="position:absolute;bottom:14px;left:12px;right:12px;">
                                    ...
                                    ${isOwner ? `<button onclick="toggleJobStatus(${item.id})" style="background:none;border:none;color:rgba(255,255,255,0.3);font-size:0.6rem;cursor:pointer;padding:4px;width:100%;text-align:center;">${isDisponible?'Marcar ocupado':'Marcar libre'}</button>` : '<div style="height:22px;"></div>'}
                                </div>
                            </div>`;
                            ...
                            if (IS_MOBILE) return `
                            <div class="glass" data-candidate-id="${item.id}" style="border-radius:20px;padding:14px 12px 60px;display:flex;flex-direction:column;align-items:center;text-align:center;gap:10px;animation-delay:${i*0.05}s;border:1px solid rgba(255,215,0,0.18);position:relative;overflow:hidden;height:330px;box-sizing:border-box;">
                                ...
                                <div style="position:absolute;bottom:14px;left:12px;right:12px;">
                                    ...
                                    ${isOwner ? `<button onclick="toggleCandidateStatus(${item.id})" style="background:none;border:none;color:rgba(255,255,255,0.3);font-size:0.6rem;cursor:pointer;padding:4px;width:100%;text-align:center;">${isCandDisponible?'Marcar ocupado':'Marcar libre'}</button>` : '<div style="height:22px;"></div>'}
                                </div>
                            </div>`;
