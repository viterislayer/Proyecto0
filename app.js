    ...
    // ── DASHBOARD ──
        ...
        app.innerHTML = `
            ...
            <div style="max-width:${IS_MOBILE ? '100%' : '1400px'};margin:auto;padding:${IS_MOBILE ? (currentUser.rol === 'usuario' ? '12px 12px 84px' : '12px') : '32px 24px'};">
                ...
                <div class="${IS_MOBILE ? '' : 'grid'}" style="${IS_MOBILE ? 'display:grid;grid-template-columns:repeat(2,1fr);gap:10px;padding:14px 0 84px;grid-auto-rows:320px;' : ''}">
                    ${feedItems.map((item, i) => {
                        if(item.itemType === 'job') {
                            ...
                            if (IS_MOBILE) return `
                            <div class="glass" data-id="${item.id}" style="border-radius:20px;padding:12px 10px;display:flex;flex-direction:column;justify-content:space-between;text-align:center;animation-delay:${i*0.05}s;border:1px solid rgba(10,132,255,0.22);overflow:hidden;height:100%;background:linear-gradient(160deg,rgba(10,132,255,0.08) 0%,rgba(7,7,15,0.95) 100%);">
                                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-shrink:0;">
                                    <span style="font-size:0.52rem;font-weight:800;padding:2px 7px;border-radius:20px;background:${isDisponible?'rgba(48,209,88,0.15)':'rgba(255,59,48,0.15)'};color:${isDisponible?'#30d158':'#ff6b63'};">${isDisponible?'● Activo':'● Cerrado'}</span>
                                    <span style="font-size:0.52rem;font-weight:700;padding:2px 7px;border-radius:20px;background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.4);">${item.modalidad||'Presencial'}</span>
                                </div>
                                <div style="display:flex;flex-direction:column;align-items:center;gap:6px;flex-shrink:0;">
                                    ${hasEmpFoto
                                        ? `<img src="${empresaFoto}" style="width:50px;height:50px;border-radius:50%;object-fit:cover;border:2px solid rgba(10,132,255,0.4);box-shadow:0 3px 10px rgba(10,132,255,0.3);">`
                                        : `<div style="width:50px;height:50px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:rgba(10,132,255,0.12);color:#0a84ff;font-size:1.2rem;border:2px solid rgba(10,132,255,0.3);"><i class="fas fa-building"></i></div>`
                                    }
                                    <div style="font-size:0.75rem;font-weight:900;color:#fff;line-height:1.25;width:100%;">${(n=>(n[0]||'')+'<br><span style=\'font-size:0.68rem;color:rgba(255,255,255,0.75);font-weight:700;\'>'+( n[1]||'')+'</span>')((item.empresa_nombre||'Empresa').trim().split(' '))}</div>
                                    <div style="font-size:0.64rem;color:#0a84ff;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;">${item.titulo||'Puesto'}</div>
                                    <div style="font-size:0.56rem;color:rgba(255,255,255,0.38);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;"><i class="fas fa-map-marker-alt" style="color:#0a84ff;"></i> ${(item.ubicacion||'Sin ubicación').substring(0,20)}</div>
                                </div>
                                <div style="flex-shrink:0;">
                                    <div style="height:1px;background:linear-gradient(90deg,transparent,rgba(10,132,255,0.25),transparent);margin:8px 0;"></div>
                                    <div style="font-size:1.3rem;font-weight:900;color:#fff;letter-spacing:-0.5px;margin-bottom:8px;">${fmtSal(item.salario)}</div>
                                    <button onclick="openJobDetails(${item.id})" class="btn-premium" style="width:100%;padding:10px;font-size:0.7rem;border-radius:12px;justify-content:center;">
                            ...
                            if (IS_MOBILE) return `
                            <div class="glass" data-candidate-id="${item.id}" style="border-radius:20px;padding:12px 10px;display:flex;flex-direction:column;justify-content:space-between;text-align:center;animation-delay:${i*0.05}s;border:1px solid rgba(255,215,0,0.2);overflow:hidden;height:100%;background:linear-gradient(160deg,rgba(255,215,0,0.06) 0%,rgba(7,7,15,0.95) 100%);">
                                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-shrink:0;">
                                    <span style="font-size:0.52rem;font-weight:800;padding:2px 7px;border-radius:20px;background:${isCandDisponible?'rgba(48,209,88,0.15)':'rgba(255,59,48,0.15)'};color:${isCandDisponible?'#30d158':'#ff6b63'};">${isCandDisponible?'● Libre':'● Ocupado'}</span>
                                    <span style="font-size:0.52rem;font-weight:700;padding:2px 7px;border-radius:20px;background:rgba(255,215,0,0.08);color:rgba(255,215,0,0.55);">${cv.modalidad||'Presencial'}</span>
                                </div>
                                <div style="display:flex;flex-direction:column;align-items:center;gap:6px;flex-shrink:0;">
                                    ${hasCFoto
                                        ? `<img src="${cFoto}" style="width:50px;height:50px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,215,0,0.4);box-shadow:0 3px 10px rgba(255,165,0,0.25);">`
                                        : `<div style="width:50px;height:50px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:rgba(255,215,0,0.08);color:#FFD700;font-size:1.2rem;border:2px solid rgba(255,215,0,0.3);"><i class="fas fa-user"></i></div>`
                                    }
                                    <div style="font-size:0.75rem;font-weight:900;color:#fff;line-height:1.25;width:100%;">${(n=>(n[0]||'')+'<br><span style=\'font-size:0.68rem;color:rgba(255,255,255,0.75);font-weight:700;\'>'+( n[1]||'')+'</span>')((item.nombre||'Candidato').trim().split(' '))}</div>
                                    <div style="font-size:0.64rem;color:#FFD700;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;">${item.profesion||'Profesional'}</div>
                                    <div style="font-size:0.56rem;color:rgba(255,255,255,0.38);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;"><i class="fas fa-map-marker-alt" style="color:#FFD700;"></i> ${(cv.ubicacion||'Sin ubicación').substring(0,20)}</div>
                                </div>
                                <div style="flex-shrink:0;">
                                    <div style="height:1px;background:linear-gradient(90deg,transparent,rgba(255,215,0,0.18),transparent);margin:8px 0;"></div>
                                    <div style="font-size:1.3rem;font-weight:900;color:#fff;letter-spacing:-0.5px;margin-bottom:8px;">${cv.moneda||'Q'} ${fmtSal(item.salario_deseado)}</div>
                                    <button onclick="openCandidateDetails(${item.id})" class="btn-premium btn-yellow-gradient" style="width:100%;padding:10px;font-size:0.7rem;border-radius:12px;justify-content:center;">
