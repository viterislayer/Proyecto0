...
// --- Optimización Profesional para Teléfonos ---
(function injectMobileStyles() {
    const style = document.createElement('style');
    style.innerHTML = `
        @media (max-width: 768px) {
            .modal-content.glass { padding: 30px 20px !important; width: 92% !important; }
            .modal-large > div { flex-direction: column !important; }
            .modal-large div[style*="border-right"] { border-right: none !important; border-bottom: 1px solid rgba(255,255,255,0.05) !important; min-width: 100% !important; }
            .modal-large div[style*="flex:1.6"], .modal-large div[style*="flex:1.5"] { min-width: 100% !important; padding: 30px 20px !important; }
            .navbar { padding: 12px 15px !important; gap: 8px !important; }
            .navbar .logo { font-size: 1.1rem !important; }
            .navbar div[style*="flex-wrap:wrap"] { justify-content: space-between; width: 100%; }
            .navbar button { padding: 6px 10px !important; font-size: 0.75rem !important; }
            .avatar { width: 32px !important; height: 32px !important; }
            h1, h2 { font-size: 1.4rem !important; }
            .btn-premium { padding: 12px !important; font-size: 0.85rem !important; }
            .grid { grid-template-columns: 1fr !important; padding: 10px !important; }
            .job-card { width: 100% !important; margin-bottom: 15px; }
            #registroForm .glass, #loginForm .glass { padding: 35px 25px !important; }
        }
        input, select, textarea { font-size: 16px !important; } /* Evita zoom automático en iOS */
    `;
...
function openCVModal() {
    ...
    Object.assign(modal.style, { position: 'fixed', inset: '0', display: 'flex', justifyContent: 'center', zIndex: '10000', padding: '40px 20px', background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)' });
    modal.innerHTML = `
...
function openJobDetails(id) {
    ...
    document.body.style.overflow = 'hidden';
    Object.assign(modal.style, { position: 'fixed', inset: '0', display: 'flex', justifyContent: 'center', zIndex: '10000', padding: '40px 20px', background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)' });
    modal.innerHTML = `
        <div class="modal modal-large glass" style="margin:auto; width:min(740px,95%); max-height: 80vh; overflow-y: auto; overscroll-behavior: contain; border:1px solid rgba(10,132,255,0.25); box-shadow: 0 30px 100px rgba(0,0,0,0.9); border-radius: 28px;">
...
async function registrarUsuario(e) {
    ...
    try {
        ...
        // Verificamos si la respuesta es JSON para evitar el error de Safari
        const contentType = res.headers.get("content-type");
        if (!contentType || !contentType.includes("application/json")) {
            throw new Error("El servidor no respondió correctamente. Asegúrate de tener el backend activo (no funciona solo con Axure estático).");
        }
...
async function loginUsuario(e) {
    ...
    try {
        ...
        const contentType = res.headers.get("content-type");
        if (!contentType || !contentType.includes("application/json")) {
            throw new Error("Respuesta inválida del servidor. Verifica tu conexión.");
        }

        const result = await res.json();
        if (!res.ok) throw new Error(result.error || "Error al ingresar");
            ...
            <div style="display:flex;justify-content:center;align-items:center;min-height:100vh;padding:24px;">
                <div class="glass responsive-card" style="padding:50px 48px;width:560px;max-width:100%;">
                    ...
                    <form id="registroForm">
                        <div style="margin-bottom:20px;"><div class="input-group" style="margin:0;"><i class="fas fa-user"></i><input name="nombre" placeholder="Nombre completo" required></div></div>
                        <div style="margin-bottom:20px;"><div class="input-group" style="margin:0;"><i class="fas fa-envelope"></i><input name="correo" type="email" placeholder="Correo electrónico" required></div></div>
                        <div style="margin-bottom:20px;"><div class="input-group" style="margin:0;"><i class="fas fa-key"></i><input name="password" type="password" placeholder="Contraseña" required></div></div>
                        <div style="margin-bottom:24px;"><div class="input-group" style="margin:0;"><i class="fas fa-key"></i><input name="confirm_password" type="password" placeholder="Confirmar contraseña" required></div></div>
                        ...
                        <button type="submit" class="btn-premium" style="width:100%;padding:16px;justify-content:center;font-size:1rem; font-weight:800; text-transform:uppercase; letter-spacing:1px;">
