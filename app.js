...
function openCVModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content glass" style="margin:auto; max-width:520px; width:95%; max-height: 85vh; overflow-y: auto; overscroll-behavior: contain; padding:36px; border:1px solid rgba(58,148,255,0.18);">
            ...
            <form id="cvForm">
                ...
                <div style="margin-bottom:16px;">
                    <label style="display:block;font-size:0.75rem;color:#0a84ff;margin-bottom:8px;margin-left:16px;font-weight:700;text-transform:uppercase;letter-spacing:0.8px;">Salario mensual deseado (USD)</label>
                    <div class="input-group" style="margin:0;">
                        <i class="fas fa-dollar-sign"></i>
                        <input name="salario" type="number" placeholder="Ej: 3000" value="${currentUser.salario_deseado || ''}" min="0" required>
                    </div>
                </div>
                <div style="margin-bottom:16px;">
...
function openPublicarAnuncio() {
    ...
    Object.assign(modal.style, { position: 'fixed', inset: '0', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: '10000', padding: '20px', background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)' });
    modal.innerHTML = `
        <div class="modal-content glass" style="margin:auto; max-width:580px; width:95%; max-height: 85vh; overflow-y: auto; overscroll-behavior: contain; padding:32px; border:1px solid rgba(255,255,255,0.12);">
            ...
            <form id="publicarAnuncioForm">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:16px;">
                    ...
                    <div>
                        ...
                        <div class="input-group" style="margin:0;">
                            <i class="fas fa-dollar-sign"></i>
                            <input name="salario" placeholder="Ej: 1200 USD / 600 por semana" required>
                        </div>
                    </div>
                </div>
...
function getJobStatus(id) {
    return localStorage.getItem(`jobStatus_${id}`) || 'disponible';
}

function setJobStatus(id, status) {
    localStorage.setItem(`jobStatus_${id}`, status);
    render('dashboard');
}

function toggleJobStatus(id) {
    const newStatus = getJobStatus(id) === 'disponible' ? 'no_disponible' : 'disponible';
    setJobStatus(id, newStatus);
}

function getCandidateStatus(id) {
    return localStorage.getItem(`candidateStatus_${id}`) || 'disponible';
}

function setCandidateStatus(id, status) {
    localStorage.setItem(`candidateStatus_${id}`, status);
    render('dashboard');
}

function toggleCandidateStatus(id) {
    const newStatus = getCandidateStatus(id) === 'disponible' ? 'no_disponible' : 'disponible';
    setCandidateStatus(id, newStatus);
}

function openJobDetails(id) {
    const item = window.feedItems?.find(job => job.id === id && job.itemType === 'job');
    if (!item) return;
    const empresaFoto = (item.empresa_foto && item.empresa_foto.startsWith('http')) ? item.empresa_foto : item.empresa_foto ? '/uploads/' + item.empresa_foto : 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png';
    ...
    const duracion = item.duracion || 'Indefinida';
    const contrato = item.contrato || 'Tiempo completo';
    const requisitos = item.requisitos || 'Experiencia previa, buena comunicación y actitud proactiva.';
    const beneficios = item.beneficios || 'Trabajo en equipo, horario flexible, capacitaciones y ambiente agradable.';
    ...
    document.body.style.overflow = 'hidden';
    Object.assign(modal.style, { position: 'fixed', inset: '0', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: '10000', padding: '20px', background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)' });
    modal.innerHTML = `
        <div class="modal modal-large glass" style="margin:auto; width:min(740px,95%); max-height: 85vh; overflow-y: auto; overscroll-behavior: contain; border:1px solid rgba(10,132,255,0.25); box-shadow: 0 30px 100px rgba(0,0,0,0.9); border-radius: 28px;">
            <div style="display:flex;flex-wrap:wrap;min-height:auto;">
                ...
                <div style="flex:1.6;min-width:320px;padding:45px 40px;display:flex;flex-direction:column;gap:36px;">
                    ...
                    <div>
                        ...
                        <div style="font-size:2.6rem;font-weight:900;color:#fff;text-shadow: 0 5px 15px rgba(0,0,0,0.3); letter-spacing: -1px;">${item.salario || '$0 USD'}</div>
                    </div>
...
function openCandidateDetails(id) {
    ...
    const cFoto = (item.foto_perfil && item.foto_perfil.startsWith('http')) ? item.foto_perfil : item.foto_perfil ? '/uploads/' + item.foto_perfil : 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png';
    ...
    Object.assign(modal.style, { position: 'fixed', inset: '0', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: '10000', padding: '20px', background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)' });
    modal.innerHTML = `
        <div class="glass" style="margin:auto; width:min(760px,95%); max-height: 85vh; overflow-y: auto; overscroll-behavior: contain; border:1.5px solid rgba(255,215,0,0.3); box-shadow: 0 25px 80px rgba(0,0,0,0.8); border-radius: 28px;">
            <div style="display:flex;flex-wrap:wrap;min-height:500px;">
                ...
                <div style="flex:1;min-width:300px;padding:40px 30px;background:rgba(255,255,255,0.015);border-right:1px solid rgba(255,255,255,0.05);display:flex;flex-direction:column;align-items:center;text-align:center;position:relative;">
                    <button onclick="closeCandidateDetailsModal()" style="position:absolute;top:20px;left:20px;background:rgba(255,255,255,0.05);border:none;color:#fff;width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:10;">×</button>
                    ...
                    <div style="width:100%;display:flex;flex-direction:column;gap:10px;margin-bottom:30px;">
                        ...
                        <div style="background:rgba(255,255,255,0.04);padding:10px 15px;border-radius:12px;font-size:0.85rem;color:rgba(255,255,255,0.7);display:flex;align-items:center;gap:10px;border:1px solid rgba(255,255,255,0.05);">
                            ...
                            <span style="text-align:left;">Ubicación: <strong>${cv.ubicacion || 'Oficina'}</strong></span>
                        </div>
                        <div style="background:rgba(255,255,255,0.04);padding:10px 15px;border-radius:12px;font-size:0.85rem;color:rgba(255,255,255,0.7);display:flex;align-items:center;gap:10px;border:1px solid rgba(255,255,255,0.05);">
                ...
                <div style="flex:1.5;min-width:320px;padding:40px 35px;">
                    <div style="margin-bottom:30px;">
                        ...
                        <div style="font-size:2.4rem;font-weight:900;color:#fff;text-shadow: 0 0 20px rgba(255,255,255,0.1);">$${parseFloat(item.salario_deseado || 0).toLocaleString()} USD</div>
                    </div>
...
async function registrarUsuario(e) {
    ...
    const formData = new FormData(e.target);
    const data = Object.fromEntries(formData.entries());
    if (data.password !== data.confirm_password) { alert("❌ Las contraseñas no coinciden"); return; }
    try {
        const res = await fetch(`${API_URL}/registro`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
...
async function loginUsuario(e) {
    e.preventDefault();
    try {
        const res = await fetch(`${API_URL}/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ correo: e.target.correo.value, password: e.target.password.value }) });
        const result = await res.json();
        if (!res.ok) throw new Error(result.error);
        localStorage.setItem('token', result.token);
        window.location.reload();
...
async function render(view) {
    ...
    if(view === 'login') {
        app.innerHTML = `
            <div style="display:flex;justify-content:center;align-items:center;min-height:100vh;padding:24px;">
                <div class="glass" style="padding:48px 44px;width:440px;max-width:100%;text-align:center;">
                    <div style="margin-bottom:32px;">
                        <div style="width:72px;height:72px;background:rgba(10,132,255,0.12);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;">
                            <i class="fas fa-bolt" style="font-size:2rem;color:#0a84ff;"></i>
                        </div>
                        <h1 class="logo" style="font-size:2.2rem;margin-bottom:6px;">WorkFlow</h1>
                        <p style="color:rgba(255,255,255,0.5);font-size:0.9rem;">Accede a tu red profesional global</p>
                    </div>
                    <form id="loginForm">
                        <div class="input-group"><i class="fas fa-envelope"></i><input type="email" name="correo" placeholder="Correo electrónico" required></div>
                        <div class="input-group"><i class="fas fa-lock"></i><input type="password" name="password" placeholder="Contraseña" required></div>
                        <button type="submit" class="btn-premium" style="width:100%;margin-top:18px;padding:15px;justify-content:center;font-size:0.95rem;">
                    ...
                    <p style="color:rgba(255,255,255,0.5);font-size:0.88rem;">¿Sin cuenta? <a href="#" onclick="render('registro');return false;" style="font-weight:600;">Crear cuenta gratis →</a></p>
                </div>
            </div>
        `;
    ...
    // ── REGISTRO ──
        ...
        app.innerHTML = `
            <div style="display:flex;justify-content:center;align-items:center;min-height:100vh;padding:24px;">
                <div class="glass" style="padding:44px;width:560px;max-width:100%;">
                    <div style="text-align:center;margin-bottom:28px;">
                        <div style="width:64px;height:64px;background:rgba(10,132,255,0.12);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;">
                            <i class="fas fa-user-plus" style="font-size:1.7rem;color:#0a84ff;"></i>
                        </div>
                        <h2 style="font-weight:800;letter-spacing:-0.5px;font-size:1.6rem;margin-bottom:6px;">Únete a WorkFlow</h2>
                        <p style="color:rgba(255,255,255,0.5);font-size:0.88rem;">Conecta con miles de oportunidades laborales</p>
                    </div>
                    <form id="registroForm">
                        <div class="input-group"><i class="fas fa-user"></i><input name="nombre" placeholder="Nombre completo" required></div>
                        <div class="input-group"><i class="fas fa-at"></i><input name="usuario" placeholder="Nombre de usuario" required></div>
                        <div class="input-group"><i class="fas fa-envelope"></i><input name="correo" type="email" placeholder="Correo electrónico" required></div>
                        <div class="input-group"><i class="fas fa-key"></i><input name="password" type="password" placeholder="Contraseña" required></div>
                        <div class="input-group"><i class="fas fa-key"></i><input name="confirm_password" type="password" placeholder="Confirmar contraseña" required></div>
                        ...
                        <button type="submit" class="btn-premium" style="width:100%;margin-top:22px;padding:15px;justify-content:center;font-size:0.95rem;">
                            Crear cuenta <i class="fas fa-check-circle"></i>
                        </button>
                    </form>
                    <div style="height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.1),transparent);margin:22px 0;"></div>
                    <p style="text-align:center;color:rgba(255,255,255,0.5);font-size:0.88rem;"><a href="#" onclick="render('login');return false;">← Ya tengo una cuenta</a></p>
                </div>
            </div>
        `;
    ...
    // ── DASHBOARD ──
        ...
        try { 
            ...
            // Mezclamos empleos y talentos, ordenando por ID (lo más reciente primero)
            feedItems = [
        ...
        if (feedItems.length === 0) {
            feedItems = [{id: 0, titulo: "Bienvenido a WorkFlow", empresa_nombre: "WorkFlow Corp", ubicacion: "Remoto", salario: "$2,500 USD", itemType: 'job'}];
        }
        window.feedItems = feedItems;

        const fotoUrl = (currentUser.foto_perfil && currentUser.foto_perfil.startsWith('http')) 
            ? currentUser.foto_perfil 
            : '/uploads/' + (currentUser.foto_perfil || 'default.jpg');
            ...
            <nav class="navbar">
                ...
                <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                    <div style="display:flex;align-items:center;gap:10px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:50px;padding:6px 14px 6px 6px;">
                        <img src="${fotoUrl}" class="avatar" style="width:36px;height:36px;">
                ...
                <div class="grid">
                    ${feedItems.map((item, i) => {
                        if(item.itemType === 'job') {
                            const empresaFoto = (item.empresa_foto && item.empresa_foto.startsWith('http')) ? item.empresa_foto : item.empresa_foto ? '/uploads/' + item.empresa_foto : '';
                            const jobBadge = item.salario ? item.salario : '$0';
                            const isDisponible = getJobStatus(item.id) === 'disponible';
                            const isOwner = currentUser && String(currentUser.id) === String(item.empresa_id);
                            return `
                            <div class="glass job-card profile-card" style="animation-delay:${i * 0.06}s;border:1.5px solid ${isDisponible ? 'rgba(10,132,255,0.25)' : 'rgba(255,107,99,0.45)'};background:${isDisponible ? 'rgba(255,255,255,0.03)' : 'rgba(255,107,99,0.07)'};box-shadow: 0 12px 40px -10px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(10,132,255,0.03); ${isOwner ? 'height:100%;' : 'align-self:start; height:auto; min-height:auto;'}">
                                <div style="padding:${isOwner ? '24px 22px' : '24px 22px 24px'};display:flex;flex-direction:column;gap:0;${isOwner ? 'min-height:100%;height:100%;' : 'height:auto; min-height:auto;'}">
                                    ...
                                    <div style="text-align:center;margin-bottom:6px;">
                                        ...
                                        <div style="font-size:0.8rem;color:rgba(255,255,255,0.5);font-weight:700;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:24px;">${item.ubicacion || 'Oficina'}</div>
                                        ...
                                        <div style="font-size:2.2rem;font-weight:900;color:#fff;line-height:1;letter-spacing:-1px;text-shadow: 0 0 15px rgba(255,255,255,0.15);">${item.salario || '$0 USD'}</div>
                                        <div style="margin-top:16px;font-size:0.75rem;color:rgba(255,255,255,0.4);font-weight:600;"><i class="fas fa-users" style="margin-right:5px;color:#0a84ff;"></i>${item.num_postulaciones || 0} postulantes</div>
                                    </div>
                                    ...
                                    <div style="display:grid;gap:10px;">
                                        ...
                                        ${isOwner ? `
                                            <button onclick="toggleJobStatus(${item.id})" class="btn-premium btn-ghost" style="width:100%;padding:13px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.14);color:#fff;font-size:0.85rem;">
                                                <i class="fas fa-user-check"></i> ${isDisponible ? 'Ya conseguí personal' : 'Volver disponible'}
                                            </button>
                                        ` : ''}
                                    </div>
                            ...
                            const cv = getCVData(item);
                            const cFoto = (item.foto_perfil && item.foto_perfil.startsWith('http')) ? item.foto_perfil : item.foto_perfil ? '/uploads/' + item.foto_perfil : '';
                            ...
                            return `
                            <div class="glass job-card profile-card" data-candidate-id="${item.id}" style="animation-delay:${i * 0.06}s;border:1.5px solid ${isCandDisponible ? 'rgba(255,215,0,0.3)' : 'rgba(255,107,99,0.45)'};background:${isCandDisponible ? 'rgba(255,255,255,0.03)' : 'rgba(255,107,99,0.07)'};box-shadow: 0 12px 40px -10px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(255,215,0,0.05); ${isOwner ? 'height:100%;' : 'align-self:start; height:auto; min-height:auto;'}">
                                <div style="padding:${isOwner ? '24px 22px' : '24px 22px 24px'};display:flex;flex-direction:column;gap:0;${isOwner ? 'min-height:100%;height:100%;' : 'height:auto; min-height:auto;'}">
                                    ...
                                    <div style="text-align:center;margin-bottom:6px;">
                                        ...
                                        <div style="font-size:0.8rem;color:rgba(255,255,255,0.5);font-weight:700;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:28px;">${cv.ubicacion || 'Oficina'}</div>
                                        ...
                                        <div style="font-size:2.2rem;font-weight:900;color:#fff;line-height:1;letter-spacing:-1px;text-shadow: 0 0 15px rgba(255,255,255,0.15);">$${parseFloat(item.salario_deseado || 0).toLocaleString()} USD</div>
                                    </div>
