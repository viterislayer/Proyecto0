    ...
    // ── DASHBOARD ──
        ...
        app.innerHTML = `
            ...
            <div style="max-width:${IS_MOBILE ? '100%' : '1400px'};margin:auto;padding:${IS_MOBILE ? (currentUser.rol === 'usuario' ? '12px 12px 84px' : '12px') : '32px 24px'};">
                ...
                <div class="${IS_MOBILE ? '' : 'grid'}" style="${IS_MOBILE ? 'display:grid;grid-template-columns:repeat(2,1fr);gap:10px;padding:14px 0;grid-auto-rows:310px;' : ''}">
                    ${feedItems.map((item, i) => {
                        if(item.itemType === 'job') {
                            ...
                            if (IS_MOBILE) return `
                            <div class="glass" data-id="${item.id}" style="border-radius:22px;padding:16px 14px;display:flex;flex-direction:column;justify-content:space-between;text-align:center;animation-delay:${i*0.05}s;border:1px solid rgba(10,132,255,0.2);position:relative;overflow:hidden;height:100%;background:linear-gradient(160deg,rgba(10,132,255,0.07) 0%,rgba(7,7,15,0.9) 100%);">
                                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                                    <span style="font-size:0.55rem;font-weight:800;padding:3px 9px;border-radius:20px;background:${isDisponible?'rgba(48,209,88,0.15)':'rgba(255,59,48,0.15)'};color:${isDisponible?'#30d158':'#ff6b63'};">${isDisponible?'● Activo':'● Cerrado'}</span>
                                    <span style="font-size:0.55rem;font-weight:700;padding:3px 9px;border-radius:20px;background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.45);">${item.modalidad||'Presencial'}</span>
                                </div>
                                <div style="display:flex;flex-direction:column;align-items:center;gap:8px;">
                                    ${hasEmpFoto
                                        ? `<img src="${empresaFoto}" style="width:56px;height:56px;border-radius:50%;object-fit:cover;border:2px solid rgba(10,132,255,0.4);box-shadow:0 4px 14px rgba(10,132,255,0.25);">`
                                        : `<div style="width:56px;height:56px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:rgba(10,132,255,0.12);color:#0a84ff;font-size:1.4rem;border:2px solid rgba(10,132,255,0.3);box-shadow:0 4px 14px rgba(10,132,255,0.15);"><i class="fas fa-building"></i></div>`
                                    }
                                    <div style="font-size:0.75rem;font-weight:900;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;">${item.empresa_nombre||'Empresa'}</div>
                                    <div style="font-size:0.68rem;color:#0a84ff;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;">${item.titulo||'Puesto'}</div>
                                    <div style="font-size:0.6rem;color:rgba(255,255,255,0.4);"><i class="fas fa-map-marker-alt" style="color:#0a84ff;font-size:0.55rem;"></i> ${(item.ubicacion||'Sin ubicación').substring(0,18)}</div>
                                </div>
                                <div>
                                    <div style="height:1px;background:linear-gradient(90deg,transparent,rgba(10,132,255,0.2),transparent);margin:10px 0;"></div>
                                    <div style="font-size:1.35rem;font-weight:900;color:#fff;letter-spacing:-0.5px;margin-bottom:10px;">${fmtSal(item.salario)}</div>
                                    <button onclick="openJobDetails(${item.id})" class="btn-premium" style="width:100%;padding:11px;font-size:0.72rem;border-radius:14px;justify-content:center;letter-spacing:0.3px;">
                                        <i class="fas fa-paper-plane"></i> Postularme
                                    </button>
                                    ${isOwner ? `<button onclick="toggleJobStatus(${item.id})" style="background:none;border:none;color:rgba(255,255,255,0.25);font-size:0.55rem;cursor:pointer;padding:5px;width:100%;text-align:center;">${isDisponible?'Cerrar vacante':'Reactivar'}</button>` : ''}
                                </div>
                            </div>`;
                            ...
                            if (IS_MOBILE) return `
                            <div class="glass" data-candidate-id="${item.id}" style="border-radius:22px;padding:16px 14px;display:flex;flex-direction:column;justify-content:space-between;text-align:center;animation-delay:${i*0.05}s;border:1px solid rgba(255,215,0,0.18);position:relative;overflow:hidden;height:100%;background:linear-gradient(160deg,rgba(255,215,0,0.05) 0%,rgba(7,7,15,0.9) 100%);">
                                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                                    <span style="font-size:0.55rem;font-weight:800;padding:3px 9px;border-radius:20px;background:${isCandDisponible?'rgba(48,209,88,0.15)':'rgba(255,59,48,0.15)'};color:${isCandDisponible?'#30d158':'#ff6b63'};">${isCandDisponible?'● Libre':'● Ocupado'}</span>
                                    <span style="font-size:0.55rem;font-weight:700;padding:3px 9px;border-radius:20px;background:rgba(255,215,0,0.08);color:rgba(255,215,0,0.6);">${cv.modalidad||'Presencial'}</span>
                                </div>
                                <div style="display:flex;flex-direction:column;align-items:center;gap:8px;">
                                    ${hasCFoto
                                        ? `<img src="${cFoto}" style="width:56px;height:56px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,215,0,0.4);box-shadow:0 4px 14px rgba(255,165,0,0.2);">`
                                        : `<div style="width:56px;height:56px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:rgba(255,215,0,0.08);color:#FFD700;font-size:1.4rem;border:2px solid rgba(255,215,0,0.3);box-shadow:0 4px 14px rgba(255,165,0,0.12);"><i class="fas fa-user"></i></div>`
                                    }
                                    <div style="font-size:0.75rem;font-weight:900;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;">${item.nombre||'Candidato'}</div>
                                    <div style="font-size:0.68rem;color:#FFD700;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;">${item.profesion||'Profesional'}</div>
                                    <div style="font-size:0.6rem;color:rgba(255,255,255,0.4);"><i class="fas fa-map-marker-alt" style="color:#FFD700;font-size:0.55rem;"></i> ${(cv.ubicacion||'Sin ubicación').substring(0,18)}</div>
                                </div>
                                <div>
                                    <div style="height:1px;background:linear-gradient(90deg,transparent,rgba(255,215,0,0.15),transparent);margin:10px 0;"></div>
                                    <div style="font-size:1.35rem;font-weight:900;color:#fff;letter-spacing:-0.5px;margin-bottom:10px;">${cv.moneda||'Q'} ${fmtSal(item.salario_deseado)}</div>
                                    <button onclick="openCandidateDetails(${item.id})" class="btn-premium btn-yellow-gradient" style="width:100%;padding:11px;font-size:0.72rem;border-radius:14px;justify-content:center;letter-spacing:0.3px;">
                                        <i class="fas fa-user-check"></i> Ver Talento
                                    </button>
                                    ${isOwner ? `<button onclick="toggleCandidateStatus(${item.id})" style="background:none;border:none;color:rgba(255,255,255,0.25);font-size:0.55rem;cursor:pointer;padding:5px;width:100%;text-align:center;">${isCandDisponible?'Marcar ocupado':'Marcar libre'}</button>` : ''}
                                </div>
                            </div>`;
