...
const initDB = async () => {
    try {
        ...
        try {
            ...
            const [durCols] = await db.query("SHOW COLUMNS FROM empleos LIKE 'duracion'");
            if (durCols.length === 0) {
                await db.query("ALTER TABLE empleos ADD COLUMN duracion VARCHAR(100) DEFAULT NULL");
            }
            const [reqCols] = await db.query("SHOW COLUMNS FROM empleos LIKE 'requisitos'");
            if (reqCols.length === 0) {
                await db.query("ALTER TABLE empleos ADD COLUMN requisitos TEXT DEFAULT NULL");
            }
            const [benCols] = await db.query("SHOW COLUMNS FROM empleos LIKE 'beneficios'");
            if (benCols.length === 0) {
                await db.query("ALTER TABLE empleos ADD COLUMN beneficios TEXT DEFAULT NULL");
            }
...
app.put('/api/cv', verificarToken, upload.single('cvFile'), async (req, res) => {
    try {
        ...
        let cvActual = rows[0]?.cvInfo || {};
        if (typeof cvActual === 'string') { try { cvActual = JSON.parse(cvActual); } catch(e) { cvActual = {}; } }
        const cvData = JSON.stringify({
            bio, skills, link,
            file: fileName || cvActual.file || null,
            publicado: true,
            moneda: moneda || 'Q',
            ubicacion,
            disponible: cvActual.disponible !== false
        });
        await db.query(
...
app.post('/api/empleos', verificarToken, async (req, res) => {
    const { titulo, descripcion, salario, ubicacion, modalidad, duracion, requisitos, beneficios } = req.body;
    ...
    try {
        await db.query(
            'INSERT INTO empleos (titulo, descripcion, salario, ubicacion, modalidad, empresa_id, duracion, requisitos, beneficios) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [titulo, descripcion, salario, ubicacion, modalidad, req.user.id, duracion || null, requisitos || null, beneficios || null]
        );
