...
const JWT_SECRET = process.env.JWT_SECRET || 'workflow_token_secret_2024_premium';
...
// 1. Configuración de CORS explícita para evitar 405 en preflight OPTIONS
app.use(cors({
    origin: '*', // En producción puedes cambiarlo por tu dominio de Azure
    methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
    allowedHeaders: ['Content-Type', 'Authorization'],
    credentials: true
}));

// 2. Middleware de parsing antes de cualquier ruta
...
// Manejo explícito de peticiones OPTIONS (Preflight) para Azure
app.options('*', cors());
...
app.post('/api/denuncias', verificarToken, async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Error al procesar denuncia' });
    }
});
...
initDB().then(() => {
    ...
    const server = app.listen(PORT, () => {
        ...
        if (err.code === 'EADDRINUSE') {
            ...
            console.error(`👉 SOLUCIÓN: Cierra la otra terminal que tengas abierta o escribe en la consola: npx kill-port ${PORT}`);
            process.exit(1);
        }
    });
});